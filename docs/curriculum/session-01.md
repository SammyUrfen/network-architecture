...
- **Minutes:** 22.
- **Big idea:** Before your code runs, the port number and a waiting line for new connections decide what a client sees: a refusal, a wait with no reply, or no answer at all.
...
- *Ports.* A big clinic building has many numbered rooms. Each doctor rents one room, and a patient asks for a doctor by the room number. The rooms from 1 to 1023 open only with the master key. Any doctor can rent a room from 1024 up. A doctor talks to many strangers, and one could trick the doctor into giving the key away, so a careful building does not give the master key to a busy doctor. A greeter with the key stands in room 80 and sends each patient up to the doctor in room 8080. Mapping: the building is one computer, a room number is a port, a doctor is a server program, a patient is a client, renting a room is bind(), the master key is root, the stranger who tricks the doctor is an attacker who uses a bug in the request code, the greeter is a front proxy. Where this breaks: root can do far more than open doors. It can read every file and change every program.
...
- *The restart trap.* The doctor leaves and comes back at once. The clinic keeps a room locked for one minute after each visit, because a late letter for the last patient could reach a new patient. The desk says that the room is in use. The doctor ignores the answer and sits in any free room, so patients who ask for room 2026 hear that nobody works there. The fix is a note on every visit: give me my room anyway. Mapping: leaving and coming back is a quick restart, the locked minute is TIME_WAIT, the late letter is a late packet, the desk's answer is EADDRINUSE, the doctor who ignores the answer is the unchecked bind(), the free room is the port that listen() picks, the note is SO_REUSEADDR. Where this breaks: the lock belongs to the old visit, not the room, and Linux accepts the note only if the earlier visit also gave it.
...
- Part 3: a friend leaves the nice way ("bye", FIN) or the hard way (runs off, RST). A helper on your side of the wall stops the first throw (ECONNRESET, no byte sent), and the teacher sends you inside on the second (SIGPIPE). Both stand for the kernel of the server. Where this breaks: the helper warns before the throw, but the kernel warns only at a write, and only one time.
...
3. `mcq`: in the demo on Linux, which call raises SIGPIPE? Answer: the second write. Distractor: the first write (S01-M11). Distractor: the read before the writes (S01-M54). Feedback: the first write returns ECONNRESET. The slide 13 comment puts SIGPIPE on the first write, but a Linux run shows it on the second. The page does not tell the learner to expect the slide wording, because a question about which document says what breaks lesson contract rule 11 (learner review, 2026-09-14).
...
2. A web page arrives with a few lines of notes in front of it. How does the reader know where the notes stop and the page starts? *Answer: an empty line ends the header lines, so the reader looks for CR LF CR LF.*
3. One of the notes in front of a web page says how long the page is, 219 bytes. What does the reader do with that number? *Answer: it takes exactly 219 bytes of page, then stops. HTTP writes the note as `Content-Length: 219`.*
...
- *Part 1.* Your friend makes two bead bracelets and sends them in a thin tube, all beads on one long string. You take beads off a handful at a time, and one handful can hold the end of the first bracelet and the start of the second. Nothing on the string shows where the first bracelet stopped. A bead is a byte, a bracelet is a message, the tube is the TCP connection, the string is the byte stream, each write() adds beads, each read() takes one handful, and the size of your hand is the space that read() gets. Where this breaks: a real string can snap and drop beads, but TCP never loses a byte and never changes their order.
- *Part 2.* You agree on a rule first. Rule one: every bracelet has exactly 10 beads, and plain wooden beads fill a short one. Rule two: a big red bead ends each bracelet. One day a bracelet has a red bead in its pattern, and it becomes two. The count of 10 is the fixed length N, the wooden beads are pad bytes, and the red bead is the delimiter. One rule counts, and the other looks for a marker, so they are two different answers. A red bead in the pattern is a newline inside a message, and the two halves are two messages. Where this breaks: a person can see which beads are wooden filler, but a program cannot. In this lesson, the reader keeps the pad bytes as part of each message.
- *Part 3.* A small card before each bracelet says how many beads follow. You read the card, then take exactly that many beads, so a bracelet can hold red beads too. The card is the length prefix. Where this breaks: a person stops at a card that says 9,000 in front of a bracelet of 7 beads. A reader trusts the number, so a wrong length breaks every later message.
...
4. `numeric`: largest message a 1-byte length prefix can describe? Answer: 255.
