...
const LENSES = [
  ...
  { key: 'pedagogy', prompt: `LENS: PEDAGOGY AND EXECUTABILITY. A later session with no memory must build each module page from this file alone. Check every template field per module, pretest answers, rung 1 fit for a 10-year-old: a real-life scene that a child knows, mapped part by part to the real thing, with a named break, worked, faded and your-turn problems with answers, checks with one unambiguous answer and distractors tied to misconception IDs, checks that ask only what the module teaches and never which slide or deck says what, one-fact cards, buildable interactive specs, valid module, prereq and thread IDs, 15-25 minute scope, real exam depth in rung 4. Flag anything that solves graded work. Run \`${LINT} <file>\` and report a score of 2.5 or more.` },
]
