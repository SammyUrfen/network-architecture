...
// The visual of Part 3: the class demo from inside, on Linux over loopback.
// Three columns: the server program, the kernel of the server, and the
// client. The client sends hello and closes with RST at once. The kernel
// marks the connection and keeps hello. The server reads, sleeps, and then
// the first write gets ECONNRESET and the second gets EPIPE and SIGPIPE.
// The SVG is drawn at the end frame of step 1: hello waits in the kernel,
// and every later part is hidden.

const STEPS = [
  { caption: 'The client sends hello. The 5 bytes wait in the kernel of the server until the server reads them.', ms: 1200 },
  { caption: 'Right after that, the client sets linger 0 and calls close(). Its kernel sends RST, not FIN, and the client ends.', ms: 1600 },
  { caption: 'The RST reaches the kernel of the server. The kernel marks the connection as reset, and it still keeps hello.', ms: 1000 },
  { caption: 'The server reads the 5 bytes of hello and sleeps for 1 second. The read gives no sign of the reset.', ms: 1200 },
...
export default function ResetLanesIsland() {
  ...
  const player = useTimeline(TIMELINE, () => {
    ...
    // A chip that shows at one x offset, travels to another, and hides.
    ...
    return [
      appear(0, 'hello'),
      appear(1, 'c-close', 0, 300),
      fly(1, 'rst', 0, KERNEL_RIGHT - CLIENT_LEFT - 20, 400, 1300),
      appear(1, 'c-ended', 1200, 1600),
      vanish(2, 'open', 0, 400),
      appear(2, 'mark', 0, 400),
      appear(3, 'read', 0, 300),
      vanish(3, 'hello', 0, 400),
      appear(3, 'sleep', 600, 900),
      appear(4, 'w1', 0, 300),
      fly(4, 'err1', 0, PROGRAM_EDGE - KERNEL_LEFT - 20, 300, 1000),
      appear(4, 'w1-result', 900, 1300),
      vanish(5, 'mark', 0, 300),
      appear(5, 'dead', 0, 300),
      appear(5, 'w2', 0, 300),
      appear(5, 'w2-result', 400, 700),
      fly(5, 'signal', 0, PROGRAM_EDGE - KERNEL_LEFT - 30, 700, 1500),
      appear(6, 'killed', 0, 500),
    ];
  });
  ...
  return (
    <AnimationControls title="Inside the demo: a reset, then two writes" captions={CAPTIONS} {...player}>
      <svg
        ...
        viewBox="0 0 360 290"
        role="img"
        aria-label="Three columns: the server program, the kernel of the server, and the client. The client sends hello and then RST. The kernel marks the connection as reset. The server reads hello. The first write gets ECONNRESET. The second write gets EPIPE and SIGPIPE, and the server ends with exit status 141."
      >
        ...
        {/* The kernel: the connection of the server, its state, and the bytes that wait */}
        <rect class="s1m3-call" x="134" y="38" width="92" height="30" rx="4" />
        <text class="s1m3-mono s1m3-mid" x="180" y="57">
          connection 4
        </text>
        <text data-part="open" class="s1m3-status" x="180" y="87">
          open
        </text>
        ...
        <g data-part="hello">
          <rect class="s1m3-chip" x="158" y="100" width="44" height="18" rx="3" />
          <text class="s1m3-chip-label" x="180" y="113">
            hello
          </text>
        </g>
        <g data-part="err1" style={{ opacity: 0 }}>
          <rect class="s1m3-chip s1m3-chip-warn" x={KERNEL_LEFT + 8} y="130" width="20" height="16" rx="3" />
          <text class="s1m3-chip-label" x={KERNEL_LEFT + 18} y="142">
            !
          </text>
        </g>
        ...
        <g data-part="c-close" {...hidden}>
          <text class="s1m3-mono" x={CLIENT_LEFT + 8} y="80">
            linger 0
          </text>
        ...
        <g data-part="rst" style={{ opacity: 0 }}>
          <rect class="s1m3-chip s1m3-chip-bad" x={CLIENT_LEFT} y="112" width="34" height="18" rx="3" />
          <text class="s1m3-chip-label" x={CLIENT_LEFT + 17} y="125">
            RST
          </text>
        </g>
        <text data-part="c-ended" class="s1m3-status-start s1m3-strong" x={CLIENT_LEFT + 8} y="160" {...hidden}>
          ended
        </text>
      </svg>
    </AnimationControls>
