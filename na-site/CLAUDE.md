...
This folder, `na-site/`, builds a static website that re-teaches Bibek's
trimester course "Network Architecture" (CN @ Scaler), lesson by lesson.
Bibek could not follow the live lectures. The site gives a first contact that
a 10-year-old can follow, then goes deep enough for exam and design
questions. More sessions arrive each week, so the site grows one session at a
time.

**Every path in this file and in `docs/` is relative to `na-site/`.** The repo
root holds only `.github/workflows/`, `README.md`, `CLAUDE.md`, `.gitignore`
and `projects/`. Run a command in the folder that the command block names.
...
Paths below are relative to `na-site/`, except the last three rows.
...
| `tools/quote_scan.py` | finds long quotes from `sources/` in tracked files | yes |
| `site/` | the Astro site, from Phase 1 | yes |
| `../.github/workflows/` | CI and the GitHub Pages deploy. Both build `na-site/site`. | yes |
| `../.claude/workflows/session-curriculum.js` | the saved workflow that writes and reviews a curriculum file | yes |
| `../projects/` | Bibek's own course work. Its code lives in a private repo. | no, git ignores it |
...
npm run build        # static output in na-site/site/dist
...
python3 na-site/tools/pdf_salvage.py na-site/sources/session-0N/slides.pdf > na-site/sources/session-0N/slides.txt
